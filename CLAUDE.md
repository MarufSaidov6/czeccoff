...
- **Чтение текста**: Нормализация → Токенизация → ORP → Тайминг → Движок. Движок = чистое ядро `step(state,settings,dt)` (одно решение на кадр, без DOM/таймеров) + тонкая оболочка `tick()` (now/таймеры/DOM/localStorage); тренер ±10 — общий чистый `trainerAdjust`. ORP-якорь считается по реальной ширине букв через canvas `measureText` (не индексный). FB2 — через DOMParser, автодетект кодировки (вкл. windows-1251).
...
## Инварианты
- Чистые модули + юнит-тесты: deriveStreak, resolveObVariant, isExpired, shouldSubmit, step, trainerAdjust. Меняешь логику — правь тест.
